...
def extract_courses(soup_or_tag):
    courses = []
    ...
    try:
        for h3 in soup_or_tag.find_all("h3", class_="coursename"):
            ...
            if not link:
                continue
            url  = link["href"]
            name = link.string.strip() if link.string else link.get_text(strip=True)
            if url and name:
                
                courses.append((name, url))
    except Exception:
        print(f"Error while extracting courses: {Exception}")
    return courses
...
def extract_file_links(soup_or_tag, base_url, session=None):
    found_file_items = []
    processed_urls = set()
    ...
    activity_instances = (
        find_activity_instances(soup_or_tag, 'resource') +
    ...
    for instance in activity_instances:
        ...
        if 'folder/view.php' in href:
            folder_url = urljoin(base_url, href)
            processed_urls.add(folder_url)
            try:
                ...
                for span in folder_soup.find_all("span", class_="fp-filename"):
                    ...
                    if tag:
                        ...
                        if file_url not in processed_urls:
                            found_file_items.append((file_name, file_url))
                            processed_urls.add(file_url)
            except Exception as e:
                print(f"Error scraping folder content: {e}")
            continue

        full_url = urljoin(base_url, href)

        instancename_span = instance.find('span', class_='instancename')
        if not instancename_span:
            continue
        ...
        if accesshide_span and accesshide_span.get_text(strip=True) in name:
            name = name.replace(accesshide_span.get_text(strip=True), '').strip()
        if not name:
            name = instancename_span.contents[0].strip() if instancename_span.contents else "Unknown File"

        if (
            '/mod/resource/view.php' in full_url or
            '/mod/assign/view.php' in full_url or
            '/pluginfile.php/' in full_url
        ):
            if full_url not in processed_urls:
...
def extract_course_urls(page_url, session):
    courses = []
    seen = set()
    try:
    ...
    container = (soup.find('div', role='main') or
                 soup.find('div', class_='content') or
                 soup)

    for a in container.select('h3.coursename a.aalink[href]'):
        href = a['href']
        if not href or href.startswith(('#', 'javascript:')):
            continue
        abs_url = urljoin(page_url, href)
        name = a.get_text(strip=True) or 'Unknown Course'
        if abs_url not in seen:
            courses.append((name, abs_url))
            seen.add(abs_url)
